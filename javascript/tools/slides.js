   // --- KEY DEOBFUSCATION ---
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        async function generatePresentation() {
            const title = document.getElementById('slides-title').value.trim();
            const content = document.getElementById('slides-content').value.trim();
            ...
            try {
                ...
                statusEl.textContent = 'Creating presentation...';
                const presentation = await gapi.client.slides.presentations.create({ title });
                ...
                if (rpcError) {
                    console.error('Critical: Google Slides was created but usage count failed to update:', rpcError.message);
                }
                ...
                try {
                    ...
                    const slideId = 'fallback_slide';
                    const titleId = 'fallback_title';
                    const bodyId = 'fallback_body';
                ...
                // Deleting the initial default slide
                if (requests.length > 0) {
                    requests.push({
                        deleteObject: {
                            objectId: presentation.result.slides[0].objectId
                        }
                    });
                ...
                statusEl.innerHTML = `Presentation created! <a href="https://docs.google.com/presentation/d/${presentationId}/edit" target="_blank" class="text-[#f97316] hover:underline">Open Presentation</a>`;
        ...
        async function enhanceWithAI() {
            ...
            if (!originalContent) {
                statusEl.textContent = 'Please enter a description to generate slides.';
                statusEl.style.color = '#ef4444';
                return;
            }

            const prompt = `Based on the following description, create a presentation. Return a JSON array of objects, where each object represents a slide and has 'title' and 'body' properties. The body can include bullet points using '\\n- '. Description: "${originalContent}"`;

            statusEl.textContent = 'AI is building your presentation...';
            ...
            try {
                ...
                try {
                    ...
                    statusEl.textContent = 'Presentation generated successfully!';
                    statusEl.style.color = '#10b981';
                } catch(e) {
        ...
        generateSlidesBtn.addEventListener('click', generatePresentation);
